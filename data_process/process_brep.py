...
def normalize(surf_pnts, edge_pnts, corner_pnts):
    ...
    for surf_pnt in surf_pnts:
        ...
        surf_pnt_wcs = (surf_pnt - global_offset[np.newaxis, np.newaxis, :]) / (
            global_scale * 0.5
        )
        ...
        pnt_ncs = (surf_pnt_wcs - local_offset[np.newaxis, np.newaxis, :]) / (
            local_scale * 0.5
        )
...
def parse_solid(solid):
    ...
    face_pnts, edge_pnts, edge_corner_pnts, edgeFace_IncM, faceEdge_IncM = (
        extract_primitive(solid)
    )
    ...
    for corner_pnt in corner_wcs.reshape(-1, 3):
        if len(corner_unique) == 0:
            ...
            if exists:
                ...
                corner_unique = np.concatenate(
                    [corner_unique, corner_pnt.reshape(1, 3)], 0
                )

    # Edge-corner adjacency
    edgeCorner_IncM = []
    for edge_corner in corner_wcs:
        start_corner_idx = np.where((corner_unique == edge_corner[0]).all(axis=1))[
            0
        ].item()
        end_corner_idx = np.where((corner_unique == edge_corner[1]).all(axis=1))[
            0
        ].item()
...
def process(step_folder):
    ...
    try:
        ...
        # Check single solid
        cad_solid = Compound.load_from_step(step_path)
        if len(cad_solid) != 1:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interval", type=int, help="Data range index, only required for abc/deepcad"
    )
